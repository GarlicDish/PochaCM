...
<h1 class="mt-4">Primary Unit List</h1>
		...
		<tbody>
			<c:forEach items="${primaryUnitList }"  var="i" >
				<tr>
					<td>${i.RNUM }</td>
					<td>${i.ITEM_NAME }</td>
					<td>${i.PRIMARY_UNIT }</td>
					<td>${i.PRIMARY_UNIT_QTY }</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
...
<div class="text-center" id="searchPanel" style="padding:10px;">
	<form id="searchForm" name="searchForm" action="/primaryUnit?curPage=1&category=${category }&keyword=${keyword }" method="get" >
...
<%@ include file="../layout/primaryUnitPaging.jsp" %>
